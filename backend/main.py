...
def init_translations(cursor):
    ...
    translations = [
        ...
        # Connect IDE translations
        ('ru', 'connect.title', 'Connect to IDE', None),
        ('ru', 'connect.description', 'Подключи MCP сервер к своему редактору одной кнопкой. После подключения перезапусти IDE.', None),
        ('ru', 'connect.status_connected', 'Подключён', None),
        ('ru', 'connect.status_disconnected', 'Не подключён', None),
        ('ru', 'connect.status_not_installed', 'Не установлен', None),
        ('ru', 'connect.btn_connect', 'Подключить', None),
        ('ru', 'connect.btn_disconnect', 'Отключить', None),
        ('ru', 'connect.btn_not_installed', 'Не установлен', None),
        ('ru', 'connect.btn_connecting', 'Подключаю...', None),
        ('ru', 'connect.manual_title', 'Ручная настройка', None),
        ('ru', 'connect.manual_description', 'Если твоего IDE нет в списке — скопируй конфиг и добавь вручную в секцию', None),
        ('ru', 'connect.copy', 'Копировать', None),
        ('ru', 'connect.copied', 'Скопировано в буфер обмена', None),
        ('ru', 'connect.mcp_missing', '⚠ MCP сервер не найден. Убедись что установка прошла корректно.', None),
        ('ru', 'connect.scanning', 'Сканирование...', None),
        ('ru', 'connect.error_load', 'Ошибка загрузки', None),
        ('ru', 'connect.error', 'Ошибка', None),
        ('ru', 'connect.restart_ide', 'подключён. Перезапусти IDE.', None),
        ('ru', 'connect.disconnected_from', 'Отключено от', None),
        ('en', 'connect.title', 'Connect to IDE', None),
        ('en', 'connect.description', 'Connect the MCP server to your editor with one click. Restart your IDE after connecting.', None),
        ('en', 'connect.status_connected', 'Connected', None),
        ('en', 'connect.status_disconnected', 'Not connected', None),
        ('en', 'connect.status_not_installed', 'Not installed', None),
        ('en', 'connect.btn_connect', 'Connect', None),
        ('en', 'connect.btn_disconnect', 'Disconnect', None),
        ('en', 'connect.btn_not_installed', 'Not installed', None),
        ('en', 'connect.btn_connecting', 'Connecting...', None),
        ('en', 'connect.manual_title', 'Manual setup', None),
        ('en', 'connect.manual_description', 'If your IDE is not in the list — copy the config and add it manually to the', None),
        ('en', 'connect.copy', 'Copy', None),
        ('en', 'connect.copied', 'Copied to clipboard', None),
        ('en', 'connect.mcp_missing', '⚠ MCP server not found. Make sure the installation completed correctly.', None),
        ('en', 'connect.scanning', 'Scanning...', None),
        ('en', 'connect.error_load', 'Load error', None),
        ('en', 'connect.error', 'Error', None),
        ('en', 'connect.restart_ide', 'connected. Restart your IDE.', None),
        ('en', 'connect.disconnected_from', 'Disconnected from', None),
        ('zh', 'connect.title', '连接 IDE', None),
        ('zh', 'connect.description', '一键将 MCP 服务器连接到您的编辑器。连接后请重启 IDE。', None),
        ('zh', 'connect.status_connected', '已连接', None),
        ('zh', 'connect.status_disconnected', '未连接', None),
        ('zh', 'connect.status_not_installed', '未安装', None),
        ('zh', 'connect.btn_connect', '连接', None),
        ('zh', 'connect.btn_disconnect', '断开', None),
        ('zh', 'connect.btn_not_installed', '未安装', None),
        ('zh', 'connect.btn_connecting', '连接中...', None),
        ('zh', 'connect.manual_title', '手动配置', None),
        ('zh', 'connect.manual_description', '如果您的 IDE 不在列表中 — 复制配置并手动添加到', None),
        ('zh', 'connect.copy', '复制', None),
        ('zh', 'connect.copied', '已复制到剪贴板', None),
        ('zh', 'connect.mcp_missing', '⚠ 未找到 MCP 服务器。请确认安装已正确完成。', None),
        ('zh', 'connect.scanning', '扫描中...', None),
        ('zh', 'connect.error_load', '加载错误', None),
